...
def assign_dist_cert(profile, dist_cert)
  ...
  {
    provisioningProfileId: profile.id,
    provisioningProfile: Base64.encode64(profile),
  }
end
...
def list(bundle_id, is_enterprise)
  ...
  if is_enterprise == 'true'
    certs = Spaceship::Portal.provisioning_profile.in_house.find_by_bundle_id(bundle_id: bundle_id)
  else 
    certs = Spaceship::Portal.provisioning_profile.app_store.find_by_bundle_id(bundle_id: bundle_id)
  end
...
def list_dist_certs(dist_certs)
  certs.map{ |cert|
    {
      id: cert.id,
      name: cert.name,
...
with_captured_stderr{
  begin
    ...
    if $action == 'create'
      ...
      provisioningProfile = create(cert)
