...
exports.getDefaultTargetAsync = getDefaultTargetAsync;
exports.isBareWorkflowProjectAsync = isBareWorkflowProjectAsync;
...
async function getDefaultTargetAsync(projectDir) {
  const {
    exp
  ...
  if (exp.sdkVersion && _semver().default.lt(exp.sdkVersion, '37.0.0')) {
    return 'managed';
  }
...
async function isBareWorkflowProjectAsync(projectDir) {
  const {
    pkg
  } = (0, _config().getConfig)(projectDir, {
    skipSDKVersionRequirement: true
  ...
  if (_fsExtra().default.existsSync(_path().default.resolve(projectDir, 'ios'))) {
    const xcodeprojFiles = await (0, _globPromise().default)(_path().default.join(projectDir, 'ios', '/**/*.xcodeproj'));
  ...
  if (_fsExtra().default.existsSync(_path().default.resolve(projectDir, 'android'))) {
    const gradleFiles = await (0, _globPromise().default)(_path().default.join(projectDir, 'android', '/**/*.gradle'));
